@foreach($records as $record)
    <tr id="tr_{{$record->id}}">
        ...
        <td><a href="#"
               class="text-dark fw-bold text-hover-primary d-block fs-6">{{$record->tipoContratto->gestore->nome}}</a><span
                    class="text-muted fw-semibold text-muted d-block fs-7">{{$record->tipoContratto->nome}}</span>
        </td>
        ...
        <td>
            <div class="d-flex justify-content-end flex-shrink-0">
                @if($record->esito_id!=='bozza')
                    <a href="{{action([\App\Http\Controllers\Backend\ModalController::class,'show'],['modifica-stato',$record->id,'aggiorna'=>'dash'])}}"
                       class="btn btn-icon btn-bg-light btn-active-color-primary btn-sm me-1"
                       data-target="kt_modal" data-toggle="modal-ajax">
                                        <span
                                                class="svg-icon svg-icon-3"><svg width="24" height="24"
                ...
                <a href="{{action([\App\Http\Controllers\Backend\ContrattoTelefoniaController::class,'edit'],$record->id)}}"
                   class="btn btn-icon btn-bg-light btn-active-color-primary btn-sm me-1"><span
                            class="svg-icon svg-icon-3"><svg width="24" height="24" viewBox="0 0 24 24" fill="none"
                                                             xmlns="http://www.w3.org/2000/svg"
