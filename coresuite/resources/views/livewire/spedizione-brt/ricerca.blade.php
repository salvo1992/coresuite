<div>
    @php($campo='ragione_sociale_destinatario')
    @php($testo='Ragione sociale destinatario')
    @php($required=true)
    <div class="row mb-6" id="div_{{$campo}}">
        ...
        <div class="col-lg-{{isset($col)?(12-$col):8}} fv-row fv-plugins-icon-container">
            <input type="text" id="{{$campo}}" name="{{$campo}}" class="form-control form-control-solid {{$classe??''}}"
                   ...
                   {!! $altro??'' !!}
                   wire:model="ragioneSociale"
            >
            {{$ragioneSociale}}
            @if($mostra)
                <div id="infoAggiuntive" style="display: none;">Informazioni aggiuntive</div>
            @endif
